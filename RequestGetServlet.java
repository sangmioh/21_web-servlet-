package com.kh.controller; //패키지선언부 (나의 위치를 알려줌)
...
public class RequestGetServlet extends HttpServlet { //RequestGetServlet클래스 영역 시작
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException { //doGet 메소드 영역 시작 
		...
		/*
		 * GET 방식으로 요청을 했다면 해당 이 doGet 메소드가 내부적으로 호출됨
		 * 
		 * doGet 메소드의
		 * 
		 * 첫번째 매개변수인 HttpServletRequest(자료형임) 타입의 request 에는 
		 * 사용자가 요청시 전달한 내용물들이 담겨서 전달됨 (사용자가입력한 값들, 요청전송방식, 요청한 사용자의 ip 주소 등)
		 * 
		 * 두번째 매개변수인 HttpServletResponse 타입의 response 에는
		 * 이 메소드에서 사용자의 요청 처리 후 응답을 할 때 필요한 객체가 만들어져서 전달됨 (필요한 메소드만 잘 호출해서 씀)
		 * 
		 * 우선, 요청을 처리하기 위해 요청시 전달된 값을 뽑는다. (==사용자가 뷰에서 입력한 값들)
		 * request로부터 뽑을 예정 (request 의 parameter 영역 안에 존재)
		 * => key-value 세트로 담겨있음! (name속성값-value값)
		 *  
		 *  따라서 request의 parameter 영역으로부터 전달된 데이터를 뽑는 메소드를 호출해야 함 (총 2가지)
		 *  1. request.getParameter("키값") : String( 그에 해당되는 value 값 리턴 )
		 *   => 무조건 문자열형으로 반환됨 (체크박스가 아닌경우)
		 *      숫자 타입일 경우는 파싱을 한다 (Wrapper 클래스 이용)
		 *   
		 *  2. request.getParameterValues("키값") : String[] (그에 해당되는 value 값들)
		 *  => 하나의 key 값을 여러개의 value 값들을 받을 경우 (체크박스)
		 *     문자열 배열 형태로 반환됨
		 *     
		 *   
		 */
		 ...
		 //라디오버튼의 경우 체크된 것이 하나도 없을 경우 null 이 넘어옴 
		 
		 int age = Integer.parseInt(request.getParameter("age")); //"10" ->10
         // request.getParameter("age")앞에 int로 강제형변환을 할수가 없음
		 // int와 String은 타입이 다르기 때문이다.
		 // int 기본형 자료형 String 참조형 자료형
		 // Integer.parseInt => String타입의 숫자를 int타입으로 변환해주는 녀석
		 // Wrapper 로 파싱 => 주의사항) NumberFormatException 오류 발생할수 있음
		 // 숫자가 아닌 text로 입력했을 때 오류가 날수 있다. 
		 ...
		 // ["한식","중식"] / null 
		 // 체크박스의 경우 체크된 것이 하나도 없을 경우 null 이 넘어옴
		...
		// System.out.println("foods: "+ foods); //주소값
		 
		 //if 안하면 nullpointer 발생  (예외 발생하지 않게 예외처리함)
		 if(foods==null) {
		  ...
		  * 1. 자바를 이용하는 방법 : JABA 코드 내에 html 을 작성하는 방법
		  * 2. JSP를 이용하는 방법 : html 코드 내에 JAVA를 작성하는 방법
		  */
		 
		 /*
		  *  1. 자바를 이용하는 방법
		  *  장점 : Java 코드 내에 html 태그들을 작성하기 때문에 
		  *        반복문이나 조건문, 자바에서 제공하는 유용한 메소드들을 활용할 수 있다.
		  *  단점 : 코드가 복잡해지고 길어짐
		 ...
		 //     인코딩 방식은 utf-8 이라는 것을 지정
		 response.setContentType("text/html; charset=UTF-8");
		 
		 // 2) 응답하고자 하는 사용자(요청을 했었던 사용자)와의 스트림((단방향)통로)을 생성
